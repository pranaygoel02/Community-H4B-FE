...
const CommunityCategory = () => {
      ...
      <div className=' grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-7'>
        {categories.map((obj,id)=>(
          <div className='w-[192px] h-[205px] box-border py-5 px-2 lg:m-10 group sm:m-5 bg-neutral transition-all' key={obj.community}>
              <div className='pt-2 text-xl  translate-y-8 group-hover:translate-y-0 transition-all'>
              <span className={`flex-adjust-center pb-1 h-1/3 text-black`} >{obj.icon}</span>
              <p className={`pb-3 font-inter h-1/3 text-black`}>{obj.community}</p>
                </div>
  ...
  return (
    <div className='text-center bg-accent  p-10'>
